...
Both `/movies` and `/downloads` must be visible inside the container — same reason as Sonarr: having both paths on the same filesystem allows a fast move (rename) during import instead of a copy across filesystems.
